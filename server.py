...
async def notify_status():
    ready_users = [user for user in server_status["users"]
                    ...
                    if msg_type == "join":
                        ...
                        if id not in waiting:
                            ...
                            if "ws" in user["other_user"]:
                                ...
                                await asyncio.wait([
                                    ws.send(
                                        msgobj("gameload", {"diff": waiting_diff, "player": 2})),
                                    user["other_user"]["ws"].send(
                                        msgobj("gameload", {"diff": diff, "player": 1})),
                                    ws.send(msgobj("name", {
                    ...
                    if "other_user" in user and "ws" in user["other_user"]:
                        if msg_type == "songsel" or msg_type == "catjump":
                            ...
                            if user["other_user"]["action"] == "waiting":
                                ...
                                await asyncio.wait([
                                    ws.send(
                                        msgobj("gameload", {"diff": user["other_user"]["gamediff"]})),
                                    user["other_user"]["ws"].send(
                                        msgobj("gameload", {"diff": diff}))
                                ])
                            else:
                                user["action"] = "waiting"
                                user["gamediff"] = diff
